...
class MessagesController implements IController 
{
	...
	public function getmsgsAction() 
	{
		...
		$messages = $this->_mm->getMssages($topicId,
																			$pages['sqlFrom'],
																			$pages['sqlCount']);
		...
		$this->_tpl->assign(array('pageName'=>'Список сообщений',
															'topicId'=>$topicId,
															'statistic'=>$statistic,
															'topicName' => $topicName,
															'errArr'=>$errArr,
	...
	public function searchAction()
	{
		...
		$pages = $this->_pg->getPages($curPage, $count, MSGS_ON_PAGE);
		$messages = $this->_mm->getSearchMssages($query,
																			$pages['sqlFrom'],
																			$pages['sqlCount']);
		...
		$errArr = $this->_event->getErr();	
		$this->_tpl->assign(array('pageName'=>'Результаты поиска',
															'statistic'=>$statistic,
															'topicName' => $topicName,
															'errArr'=>$errArr,
															'pages'=>$pages,
